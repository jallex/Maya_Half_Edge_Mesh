...
def heMesh_to_maya(heMesh):
   ...
   #Set to keep track of which half edges we still must visit
   he_set = {he}
   #set to keep track of which half edges we've seen
   seen_set = {he.id}

   #iterate through the half edges on this face, 
   # and if a half-edge has a twin add it to he_set
   while(True):
       #we've visited all half edges, break
       if len(he_set) == 0:
           break
       else:
           #pop a half edge to visit off the set
           he = he_set.pop()
       #keep track of starting half edge so we know when we've visited the whole face
       start_he = he
